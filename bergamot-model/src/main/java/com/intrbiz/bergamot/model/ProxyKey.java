 ...
 */
@SQLTable(schema = BergamotDB.class, name = "proxy_key", since = @SQLVersion({ 4, 0, 3 }))
public class ProxyKey implements Serializable
{
    ...
    @SQLColumn(index = 1, name = "id", since = @SQLVersion({ 4, 0, 3 }))
    @SQLPrimaryKey()
    protected UUID id;

    @SQLColumn(index = 2, name = "site_id", notNull = false, since = @SQLVersion({ 4, 0, 3 }))
    @SQLForeignKey(references = Site.class, on = "id", onDelete = Action.CASCADE, onUpdate = Action.RESTRICT, since = @SQLVersion({ 4, 0, 3 }))
    protected UUID siteId;
    
    @SQLColumn(index = 3, name = "purpose", notNull = true, since = @SQLVersion({ 4, 0, 3 }))
    private String purpose;
    
    /**
     * The secret key used for authentication
     */
    @SQLColumn(index = 4, name = "secret", since = @SQLVersion({ 4, 0, 3 }))
    @SQLPrimaryKey
    private byte[] secret;
     ...
     */
    @SQLColumn(index = 5, name = "created", since = @SQLVersion({ 4, 0, 3 }))
    private Timestamp created = new Timestamp(System.currentTimeMillis());
    
    /**
     * Has this token been revoked
     */
    @SQLColumn(index = 6, name = "revoked", since = @SQLVersion({ 4, 0, 3 }))
    private boolean revoked = false;
    
    /**
     * When was it revoked
     */
    @SQLColumn(index = 7, name = "revoked_at", since = @SQLVersion({ 4, 0, 3 }))
