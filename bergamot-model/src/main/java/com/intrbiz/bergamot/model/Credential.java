 ...
 */
@SQLTable(schema = BergamotDB.class, name = "credential", since = @SQLVersion({4, 0, 0}))
@SQLUnique(name = "name_unq", columns = { "site_id", "name" })
public class Credential extends SecuredObject<CredentialMO, CredentialCfg>
{
    private static final long serialVersionUID = 1L;

    @SQLColumn(index = 1, name = "username", since = @SQLVersion({4, 0, 0}))
    protected String username;

    @SQLColumn(index = 2, name = "password", since = @SQLVersion({4, 0, 0}))
    protected String password;

    @SQLColumn(index = 3, name = "key_id", since = @SQLVersion({4, 0, 0}))
    protected String keyId;

    @SQLColumn(index = 4, name = "key_secret", since = @SQLVersion({4, 0, 0}))
