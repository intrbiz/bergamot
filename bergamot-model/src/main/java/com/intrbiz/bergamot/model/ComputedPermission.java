...
@SQLTable(schema = BergamotDB.class, name = "computed_permissions", since = @SQLVersion({4, 0, 0}) )
public class ComputedPermission implements Serializable
{
    ...
    @SQLColumn(index = 2, name = "permission", since = @SQLVersion({4, 0, 0}) )
    @SQLPrimaryKey
    private String permission;

    @SQLColumn(index = 3, name = "allowed", since = @SQLVersion({4, 0, 0}) )
