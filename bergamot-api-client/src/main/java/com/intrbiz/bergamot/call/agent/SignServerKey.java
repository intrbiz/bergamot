package com.intrbiz.bergamot.call.agent;

import java.security.PublicKey;
import java.security.cert.Certificate;
import java.util.List;
import java.util.stream.Collectors;

import org.apache.http.client.fluent.Form;
import org.apache.http.client.fluent.Response;

import com.intrbiz.bergamot.BergamotAPICall;
import com.intrbiz.bergamot.BergamotAPIException;
import com.intrbiz.bergamot.BergamotClient;
import com.intrbiz.bergamot.crypto.util.PEMUtil;

public class SignServerKey extends BergamotAPICall<List<Certificate>>
{
    private String commonName;
    
    private PublicKey key;
    
    public SignServerKey(BergamotClient client)
    {
        super(client);
    }
    
    public SignServerKey commonName(String commonName)
    {
        this.commonName = commonName;
        return this;
    }
    
    public SignServerKey publicKey(PublicKey key)
    {
        this.key = key;
        return this;
    }
    
    public List<Certificate> execute()
    {
        try
        {
            Response response = execute(post(url("/agent/sign-server-key")).addHeader(authHeader())
                     .bodyForm(
                         Form.form()
                         .add("common-name", this.commonName)
                         .add("public-key", PEMUtil.savePublicKey(this.key))
                         .build()
                     ));
            return transcoder().decodeListFromString(response.returnContent().asString(), String.class).stream()
                    .map((s)->{
                        try { return PEMUtil.loadCertificate(s); } 
                        catch (Exception e) {} 
                        return null;
                    }).collect(Collectors.toList());
        }
        catch (Exception e)
        {
            throw new BergamotAPIException("Error signing server key", e);
        }
    }
}
