...
buildah config --author='Chris Ellis <chris@intrbiz.com>' --port 8080 --workingdir '/opt/bergamot/ui' --entrypoint '[ "/usr/sbin/nginx" ]' --cmd '[ "-g", "daemon off;", "-c", "/etc/nginx/nginx.conf" ]' $ID
