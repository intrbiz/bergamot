 ...
 */
@SQLTable(schema = LamplighterDB.class, name = "check_reading", since = @SQLVersion({4, 0, 0}))
...
public class CheckReading implements Serializable
{
    ...
    @SQLColumn(index = 4, name = "name", notNull = true, since = @SQLVersion({4, 0, 0}))
    protected String name;

    @SQLColumn(index = 5, name = "summary", since = @SQLVersion({4, 0, 0}))
    protected String summary;

    @SQLColumn(index = 6, name = "description", since = @SQLVersion({4, 0, 0}))
    protected String description;
    
    @SQLColumn(index = 7, name = "unit", since = @SQLVersion({4, 0, 0}))
    protected String unit;
    
    @SQLColumn(index = 8, name = "reading_type", since = @SQLVersion({4, 0, 0}))
    protected String readingType;
    
    @SQLColumn(index = 11, name = "created", since = @SQLVersion({4, 0, 0}))
    protected Timestamp created = new Timestamp(System.currentTimeMillis());

    @SQLColumn(index = 12, name = "updated", since = @SQLVersion({4, 0, 0}))
    protected Timestamp updated = new Timestamp(System.currentTimeMillis());

    @SQLColumn(index = 13, name = "poll_interval", since = @SQLVersion({4, 0, 0}))
