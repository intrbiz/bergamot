 ...
 * StatsdClient.java
 *
 * (C) 2011 Meetup, Inc. Author: Andrew Gwozdziewycz <andrew@meetup.com>, @apgwoz
 *
 *
 *
 * Example usage:
 *
 * StatsdClient client = new StatsdClient("statsd.example.com", 8125); // increment by 1 client.increment("foo.bar.baz"); // increment by 10 client.increment("foo.bar.baz", 10); // sample rate client.increment("foo.bar.baz", 10, .1); // increment multiple keys by 1 client.increment("foo.bar.baz", "foo.bar.boo", "foo.baz.bar"); // increment multiple keys by 10 -- yeah, it's "backwards" client.increment(10, "foo.bar.baz", "foo.bar.boo", "foo.baz.bar"); // multiple keys with a sample rate client.increment(10, .1, "foo.bar.baz", "foo.bar.boo", "foo.baz.bar");
 *
 * // To enable multi metrics (aka more than 1 metric in a UDP packet) (disabled by default) client.enableMultiMetrics(true); //disable by passing in false // To fine-tune udp packet buffer size (default=1500) client.setBufferSize((short) 1500); // To force flush the buffer out (good idea to add to your shutdown path) client.flush();
 *
 *
 * Note: For best results, and greater availability, you'll probably want to create a wrapper class which creates a static client and proxies to it.
 *
...
public class StatsDExample extends TimerTask
{
    ...
    private static final Logger log = Logger.getLogger(StatsDExample.class.getName());

    private final InetSocketAddress _address;

    private final DatagramChannel _channel;

    public StatsDExample(String host, int port) throws UnknownHostException, IOException {
        this(InetAddress.getByName(host), port);
    }

    public StatsDExample(InetAddress host, int port) throws IOException {
    ...
    public synchronized boolean flush()
    {
        try
        {
            ...
            {
                log.error(String.format("Could not send entirely stat %s to host %s:%d. Only sent %d bytes out of %d bytes", sendBuffer.toString(), _address.getHostName(), _address.getPort(), nbSentBytes, sizeOfBuffer));
                return false;
            }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        while (true)
        {
            ...
            client.increment("test1");
            client.timing("test2", rnd.nextInt(250));
