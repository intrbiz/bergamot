...
public abstract class AbstractResultProcessor implements ResultProcessor
{
    ...
    public void start()
    {
        ...
        for (int i = 0; i < this.getThreads(); i++)
        {
            ...
            this.resultConsumers.add(this.workerQueue.consumeResults((r) -> { processExecuted(r); }, null));
            // consume dead checks, currently for all sites
            this.deadConsumers.add(this.workerQueue.consumeDeadChecks((e) -> { processDead(e); }, null));
        }
        // scheduler queue
